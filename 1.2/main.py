...
def window():
    window0 = Tk()
    window0.title('多边形分割小程序  1.2 macOS 版')
    ...
    def _about_():
        ...
        bg0 = ImageTk.PhotoImage(Image.open('ver1_2_about.gif'))
    ...
    def _report_():
        window3 = tk.Toplevel()
        window3.title('多边形分割小程序  1.2版  反馈系统')
        window3.geometry('800x600')
        window3.resizable(False, False)
        bg0 = ImageTk.PhotoImage(Image.open('ver1_2_report.gif'))
        canvas0 = tk.Canvas(window3, width=800, height=400, bg='white')
        canvas0.create_image(400, 200, image=bg0)
        title9_1 = tk.Label(window3,
                            text='请输入您遇到的问题，我们将认真考虑您的意见与建议！',
                            font=('Arial', 18))
        entry1 = tk.Entry(window3, width=80)
        title2 = tk.Label(window3,
                          text='如果您愿意，请您留下您的联系方式，我们将会与您联系。',
                          font=('Arial', 18))
        entry2 = tk.Entry(window3, width=80)
        report1 = tk.Button(window3, text='发送至官方邮箱', command=lambda: _sent_(entry1.get(), entry2.get()))
    ...
    title1 = tk.Label(
        canvas1, text='\n1.2 macOS 版  2021年4月8日  北京市昌平区，湖北省荆州市荆州区', font=('Arial', 17))
